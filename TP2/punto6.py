...
words_tokenize = [word for word in words_tokenize if word not in puntuactions] # Eliminar puntuaciones

# C. Normalizacion: Quitar acentos y letras "ñ" de las palabras tokenizadas
words_tokenize = [unicodedata.normalize('NFKD', word).encode('ASCII', 'ignore').decode('utf-8') for word in words_tokenize]

# D. Eliminar las stopwords del texto
stop_words = set(stopwords.words('english'))
words_filtered = [w for w in words_tokenize if not w.lower() in stop_words]

#Obtener las frecuencias luego de la tokenizacion
...
# E.  Mostrar las frecuencias
print("------------------------------------------------------------------")
print("   *** 50 palabras mas frecuentes ***")
for word, freq in list(frecuency_ordered.items())[:50]:
  print(f"{word}: {freq}")
print("------------------------------------------------------------------")
...
# F. Stemming con Lancaster. Obtener un listado de las 50 palabras más frecuentes
lancaster_stem_frequency = FreqDist(lancaster_stems)
lancaster_stem_frequency_ordered = dict(sorted(lancaster_stem_frequency.items(), key = lambda item: item[1], reverse = True))
print("   *** 50 palabras más frecuentes (basado en Stemming) ***")
for stem, freq in list(lancaster_stem_frequency_ordered.items())[:50]:
  print(f"{stem}: {freq}")
print("------------------------------------------------------------------")

# G. Lematización. Obtener un listado de las 50 palabras más frecuentes
wordnet_lemma_frequency = FreqDist(wordnet_lemmas)
wordnet_lemma_frequency_ordered = dict(sorted(wordnet_lemma_frequency.items(), key = lambda item: item[1], reverse = True))
print("   *** 50 palabras más frecuentes (basado en Lematización) ***")
for lemma, freq in list(wordnet_lemma_frequency_ordered.items())[:50]:
  print(f"{lemma}: {freq}")
print("------------------------------------------------------------------")

# H. Lematización indicando el PoS para los verbos.
# Función para obtener el PoS tag para la lematización
def get_wordnet_pos(tag):
  if tag.startswith('J'):
    ...
    return wn.NOUN # Por defecto, consideramos sustantivo

tagged_words = pos_tag(words_filtered)
verb_lemmas_with_pos = {}
lemmatizer = WordNetLemmatizer()
for word, tag in tagged_words:
  if tag.startswith('V'):
    lemma = lemmatizer.lemmatize(word, get_wordnet_pos(tag))
    if (lemma, tag) in verb_lemmas_with_pos:
      verb_lemmas_with_pos[(lemma, tag)] += 1
    else:
      verb_lemmas_with_pos[(lemma, tag)] = 1

verb_lemmas_with_pos_ordered = dict(sorted(verb_lemmas_with_pos.items(), key = lambda item: item[1], reverse = True))
print("   *** Lematización indicando el PoS para los verbos (50 más frecuentes) ***")
for (lemma, pos), freq in list(verb_lemmas_with_pos_ordered.items())[:50]:
  print(f"{lemma} ({pos}): {freq}")
print("------------------------------------------------------------------")

# I. Realizar una representación tabular de los primeros 30 tokens
lancaster = LancasterStemmer() # Inicializar el stemmer de Lancaster
lemmatizer = WordNetLemmatizer() # Inicializar el lematizador de WordNet

table_data = []
tagged_words_first_30 = pos_tag(words_filtered[:30]) # Obtener los primeros 30 tokens y sus etiquetas PoS

for i, (token, tag) in enumerate(tagged_words_first_30):
  lancaster_stem = lancaster.stem(token)
  wordnet_lemma = lemmatizer.lemmatize(token)
  verb_lemma_pos = ""
  if tag.startswith('V'):
    lemma = lemmatizer.lemmatize(token, get_wordnet_pos(tag))
    verb_lemma_pos = f"{lemma} ({tag})"
  else:
    verb_lemma_pos = "-"
