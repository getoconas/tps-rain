...
def draw_and_save_graph(graph, filename="grafo_noticias.png"):
  ...
  # Etiquetas de nodos: usa el atributo 'title' si existe, sino la URL
  labels = {node: (graph.nodes[node]['title'][:25] + '...') if len(graph.nodes[node]['title']) > 25 else graph.nodes[node]['title'] for node in graph.nodes}
  #labels = {node: graph.nodes[node].get('title', node) for node in graph.nodes}
  # Etiquetas de aristas: usa el peso
  edge_labels = nx.get_edge_attributes(graph, 'weight')

  nx.draw(graph, pos, labels=labels, with_labels=True, node_size=100, edge_color='gray', alpha=0.7, font_size=8)
